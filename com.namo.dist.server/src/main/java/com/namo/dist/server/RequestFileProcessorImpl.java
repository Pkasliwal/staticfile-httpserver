...
/**
 * Implementation for {@link RequestFileProcessor} as needed for static file
 * handling.
 */
class RequestFileProcessorImpl implements RequestFileProcessor {
	...
	public boolean isModifiedAfter(String modifiedSince, DateTimeFormatter dtFormat) {
		...
		Instant lastModifiedInstant = Instant.ofEpochMilli(sourceFile.lastModified()).atOffset(ZoneOffset.UTC)
				.toInstant();
		return !modifiedSinceInstant.equals(lastModifiedInstant)
				&& modifiedSinceInstant.toEpochMilli() < lastModifiedInstant.toEpochMilli();
	}
	...
	public String getLastModifiedTime(DateTimeFormatter dtFormat) {
		OffsetDateTime lastModifiedOffsetTime = Instant.ofEpochMilli(sourceFile.lastModified())
